...
export async function deleteSocialNetwork(id_usuario, { red_social, link }) {
    try {
        await pool.query(querys.deleteSocialNetwork, [id_usuario, red_social, link]);
        return {
            ok: true,
            message: "Se elimino la red social"
        }
    } catch (error) {
        console.log(error);
